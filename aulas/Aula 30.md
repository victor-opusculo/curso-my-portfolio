...
1. Colar a pasta `seed` do anexo desta aula dentro do diretório `data`, na raíz do projeto;
1. Criar em `data`, o arquivo `seed.ts` com:
    ...
    const defaultTags = [ "lipsum", "data", "projeto", "postagem", "portfólio" ];
    ...
    const mediaIds = await db.transaction(async tx =>
    {
        ...
        for (const [id, file] of mediasTransformed)
        {
            ...
            const result = await tx.insert(schema.media).values({ id, title: `Mídia ${file}`, mimeType: (await fileType.fileTypeFromFile(destinationFile))?.mime ?? genericMimeType, fileExtension: path.extname(file) });
    ...
    const toolsIds = await db.transaction(async tx =>
    {
            ...
            for (let i = 1; i <= 10; i++)
            {
                const result = await tx.insert(schema.tools).values({ title: lipsum(), description: lipsumLong(), logoMediaId: random(mediaIds) });
...
1. Criar na raíz do projeto o arquivo `seed.ts` com:

    ```ts
    import './data/seed';
    ```

1. Executar com `npx tsx ./seed.ts` **(Execute com o servidor Next.js fechado!)**.