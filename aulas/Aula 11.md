...
### Actions e Acesso ao banco de dados

1. Dentro de `lib`, criar o diretório `media`;
1. Criar dentro o arquivo `media.ts`:
    ...
    export async function insert(formData: FormData) {
        ...
        if (!title)
            throw new MediaUploadError(
                'title',
                'Título desta mídia não informado!'
            );
        ...
        if (!newId) throw new Error('Erro ao cadastrar mídia!');
    ...
    export async function getStream(id: number) {
        ...
        if (!mediaDr) throw new RowNotFound('media');
...
1. No mesmo diretório, criar `actions.ts`:
