...
1. Criar arquivo `.env` na raíz do projeto, com a variável de ambiente:

    ```Dotenv
    DB_FILE_NAME=file:data/my-portfolio.sqlite3
    ```

1. Criar diretório `data` na raíz do projeto.
1. Criar o arquivo `my-portfolio.sql` nesse diretório com os comandos SQL para criação do banco de dados:
    ...
    INSERT INTO settings (key, value) VALUES ('admin_password_hash', '$2a$10$bKaaAQmyKibnry/MdEfg8.HSJbkzxCMWzy3aIuf6jTUU20n8usLtu');

    -- Gerar hash inicial:
    -- node -e 'import("bcrypt").then(mod => mod.hash("12345678", 10)).then(console.log);'
    ...
    CREATE TRIGGER projects_tags_update AFTER UPDATE ON projects_tags BEGIN
        UPDATE projects_tags_fts SET tags = (SELECT group_concat(tag, ' ') FROM projects_tags WHERE projects_tags.proj_id = new.proj_id) WHERE projects_tags_fts.rowid = new.proj_id;
    END;

    CREATE TRIGGER projects_tags_delete BEFORE DELETE ON projects_tags BEGIN
    ...
    BEGIN
        UPDATE blog_posts SET published_at_utc = CURRENT_TIMESTAMP, updated_at_utc = CURRENT_TIMESTAMP WHERE rowid = NEW.rowid;
    END;
...
1. Criar banco de dados SQLite. Abrir terminal na pasta `data`:
...
1. Criar diretório para uploads de mídia, assim como seu `.gitkeep`;
1. Adicionar o conteúdo de uploads e o banco de dados ao `.gitignore`:

    ```ignore
    # .gitignore

    data/my-portfolio.sqlite3

    data/uploads/media/**/*
    !data/uploads/media/.gitkeep
    ```

1. Criar a pasta `drizzle`, também em `data`;

1. Para conveniência, scripts para excluir e recriar banco de dados:
